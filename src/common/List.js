...
class List extends Network {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      nextProps.refreshControl !== this.props.refreshControl ||
  ...
  addItemToList = (item, first = false) => {
    let newData = [];
    const { _data } = this.state.dataProvider;
    if (first) newData = [item, ..._data];
    else newData = [..._data, item];
  ...
  renderFlatListItem = ({ item, index }) => {
    if (this.state.loading && index == this.state.dataProvider._data.length) {
  ...
  onFlatListScroll = ({ nativeEvent }) => {
    ...
    if (this.props.horizontal) {
      ...
      if (this.isCloseToBottom(nativeEvent)) {
console.log("bottome ==>",this.page, this.pageCount );
    ...
    <FlatList
      ...
      contentContainerStyle={
        this.props.columns > 1
          ...
          : [paddingStyles(props), this.props.style]
      }
