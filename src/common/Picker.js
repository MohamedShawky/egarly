...
class Picker extends Network {
  ...
  setEndFetching(data) {
    ...
    if (
      ...
      (typeof setInitialValueAfterFetch === "number" ||
        typeof setInitialValueAfterFetch === "string")
    ) {
      // let { label, value, dialCode } = data.find(
      //   item => item.value === setInitialValueAfterFetch,
      // );
      const target = data.find(item => item.value == setInitialValueAfterFetch);
  ...
  onChange = ({ label, value, ...rest }, noValidate) => {
    ...
    const selectedData = this.state.data.find(item => item.value === value);
        ...
        >
          ...
          {disable ? (
            ...
            >
              ...
              {rightItems.length && !this.props.icon ? (
                <AppView
                  padding={3}
                  marginHorizontal={2}
                  backgroundColor="transparent"
